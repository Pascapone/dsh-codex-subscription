...
function createSettingsAdapter(ctx, schema, config, namespace) {
	...
	return {
		...
		setSessionSpeed(sessionId, speedMode) {
			if (id && typeof ctx.settings.mutate === "function") return ctx.settings.mutate(id, [{
				op: "set",
				path: ["sessionSpeedModes", sessionId],
				value: speedMode
			}]);
			const write = pending.then(() => settings.update({ sessionSpeedModes: {
				...settings.get().sessionSpeedModes,
				[sessionId]: speedMode
			} }));
			pending = write.catch(() => {});
			return write;
...
function openaiCodexSubscriptionProvider({ resolveSpeedMode = () => void 0, resolveOutputVerbosity = () => OUTPUT_VERBOSITY_DEFAULT, resolveContextMode = () => void 0, resolveCustomContextWindow = () => void 0, catalog, connection, compaction, runNetwork = (_area, operation) => operation() } = {}) {
	...
	const withPreferences = (model, options = {}) => {
		...
		const fast = resolveSpeedMode(options.sessionId) === "fast" && (metadata?.supportsFast ?? supportsCodexFastMode(model?.id));
...
const USER_AGENT$1 = `dsh-codex-subscription/${PACKAGE_VERSION}`;
//#endregion
...
const BUNDLED_FALLBACK_MODELS = Object.freeze([Object.freeze({
	id: "gpt-6-luna",
	name: "GPT-6 Luna",
...
function mergeModel(baseModels, remote) {
	const base = baseModels.find((model) => model.id === remote.id) ?? baseModels.find((model) => model.id === remote.templateId) ?? baseModels.find((model) => model.id !== "gpt-5.3-codex-spark") ?? baseModels[0];
...
function addBundledCodexFallbacks(baseModels) {
	const visible = baseModels.filter((model) => model.id !== "gpt-5.3-codex-spark");
	const existing = new Set(visible.map((model) => model.id));
	return [...BUNDLED_FALLBACK_MODELS.filter((model) => !existing.has(model.id)).map((model) => mergeModel(baseModels, model)).filter(Boolean), ...visible];
}
...
//#endregion
//#region src/codex-transcription.js
...
function decodeTranscriptionAudio(payload) {
	const mime = typeof payload?.mimeType === "string" ? payload.mimeType.split(";", 1)[0].trim().toLowerCase() : "";
	if (!Object.hasOwn(AUDIO_FORMATS, mime) || typeof payload?.audioBase64 !== "string" || payload.audioBase64.length > Math.ceil(26214400 / 3) * 4 + 4 || payload.audioBase64.length % 4 !== 0 || !/^[A-Za-z0-9+/]+={0,2}$/u.test(payload.audioBase64)) throw new Error("Invalid audio recording");
	const audio = Buffer.from(payload.audioBase64, "base64");
	if (audio.length === 0 || audio.length > 26214400) throw new Error("Invalid audio recording");
	return {
		audio,
		mime,
		filename: `recording.${AUDIO_FORMATS[mime]}`
	};
}
function createCodexTranscriptionProvider({ getAuth, readCredential, fetch: fetchAudio = fetch }) {
	return async ({ audio, mime, filename }, signal) => {
		...
		const encoded = new Request(CODEX_TRANSCRIPTION_URL, {
			method: "POST",
			body: form
		});
		const headers = {
		...
		if (!accountId) try {
			accountId = JSON.parse(Buffer.from(access.split(".")[1], "base64url").toString("utf8"))?.["https://api.openai.com/auth"]?.chatgpt_account_id;
		} catch {}
		...
		signal.throwIfAborted();
		if (!response.ok) throw new Error(response.status === 401 || response.status === 403 ? "ChatGPT sign-in needs to be renewed" : `ChatGPT transcription failed (HTTP ${response.status})`);
...
function createSubscriptionRpcHandler({ authHandler, usageReader, resetCreditService, preferences, runtimeManagement, diagnosticsReader, modelCatalog, originalImages, resolveInheritedOriginal, closeConnections, transcriptionEnabled, transcribeAudio }) {
	return async (endpoint, payload, signal) => {
		if (endpoint === "transcription/transcribe") {
			...
			let input;
			try {
				input = decodeTranscriptionAudio(payload);
			} catch {
				return publicError("invalid-input", "Invalid audio recording");
			}
			try {
				signal.throwIfAborted();
				return {
					ok: true,
					value: await transcribeAudio(input, signal)
				};
			} catch (error) {
				if (signal.aborted) throw error;
				return publicError("transcription-failed", error?.message === "ChatGPT subscription is not signed in" || error?.message === "ChatGPT sign-in needs to be renewed" ? error.message : "ChatGPT transcription failed. Try again.");
			}
		}
		if ([
			"runtime/status",
		...
		if (endpoint === "preferences/status" || endpoint === "preferences/update") try {
			...
			if (endpoint === "preferences/update") {
				if (Object.hasOwn(payload ?? {}, "sessionId")) {
					if (typeof payload.sessionId !== "string" || payload.sessionId.length < 1 || payload.sessionId.length > 512 || !["standard", "fast"].includes(payload.speedMode) || Object.keys(payload).some((key) => !["sessionId", "speedMode"].includes(key))) return publicError("invalid-input", "Invalid session speed preference");
...
function apply(ctx, config = {}) {
	...
	const preferences = {
		status: () => ({
			...
			[SESSION_SPEED_MODES_FIELD]: settings.get()["sessionSpeedModes"] ?? {},
