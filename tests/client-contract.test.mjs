...
test('stable speed control is persisted inside the model menu and hidden from the composer while standard', async () => {
  ...
  assert.match(client, /\.codexModelSelectSubmenu\{right:calc\(100% \+ 8px\)[^}]*background:var\(--dsw-alias-bg-layer-1,#303136\);backdrop-filter:none\}/u, 'every model picker submenu must be opaque even when the host menu token is translucent')
  assert.match(client, /\.codexModelSelectSubmenu \.codexModelSelectGroupTitle\{background:var\(--dsw-alias-bg-layer-1,#303136\)\}/u, 'sticky model group headings must not reintroduce translucency')
