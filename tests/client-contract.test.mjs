...
test('stable speed control is persisted inside the model menu and hidden from the composer while standard', async () => {
  ...
  assert.match(client, /pane === 'root' \? <>[\s\S]*?\{submenu\}/u, 'the selected pane replaces the root menu instead of opening alongside it')
  assert.match(client, /onClick=\{backToRoot\}[^>]*><IconChevronLeftOutline14/u, 'the pane has a back action')
  assert.match(client, /mobile && open \? createPortal\([\s\S]*?document\.body\)/u, 'mobile picker escapes the clipped composer into a body-mounted drawer')
  assert.match(client, /!menuRef\.current\?\.contains\(event\.target\)/u, 'the outside listener must not dismiss portalled menu clicks')
  assert.match(client, /\.codexModelSelectMobileLayer \.codexModelSelectMenu\{[^}]*left:0;right:0;bottom:0;width:100%;[^}]*max-height:min\(85dvh,700px\)/u, 'mobile drawer must span the viewport and remain height-bounded')
  assert.match(client, /\.codexModelSelectMenu\{[^}]*max-width:min\(420px,calc\(100vw - 32px\)\)[^}]*overflow-y:auto\}/u, 'the menu stays within a narrow viewport')
  assert.doesNotMatch(client, /codexModelSelectSubmenu/u, 'no side-opening submenus remain')
  assert.match(client, /background:var\(--dsw-specific-menu,var\(--dsw-alias-bg-layer-1,#303136\)\)/u, 'the menu needs an opaque fallback on older hosts')
  assert.match(client, /backdrop-filter:var\(--dsw-menu-backdrop-filter,blur\(40px\) saturate\(150%\)\)/u, 'the translucent host menu token requires its matching blur')
...
test('voice waveform replaces the draft and retains three seconds of distinct audio frames', async () => {
  ...
  const [styles, voice] = await Promise.all([read('src/client-styles.js'), read('src/voice-controller.js')])
...
test('voice controls stay in the originating session and keep full width after navigation', async () => {
  ...
  assert.match(component, /queueMicrotask\(\(\) => \{ if \(!cancelled\) onActiveChange\(true\) \}\)/u)
  assert.doesNotMatch(styles, /div:has\(> div > \.codexVoiceControls\)/u)
