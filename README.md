...
| **订阅语音转文字（实验）** | 在 **设置 → Codex 订阅 → 高级与诊断** 开启。输入框麦克风直接由本插件提供：X 丢弃录音，停止键填入草稿，箭头转写并发送。复用已有 OAuth 登录，音频发送给 ChatGPT；无需额外语音插件，默认关闭。若同时安装其他使用默认麦克风位置的语音插件，会优先显示其控件，本插件的其他功能不受影响。 |
